...
+ 过程：
  ...
  + 并发标记：GC Roots Tracing 过程
  + 重新标记：修正并发标记期间因用户程序继续运作而导致变动的记录
  + 并发清除
**** G1 收集器
+ 新生代老年代通吃。
+ 采用标记整理
...
+ HashMap的实例有俩个参数影响其性能： “初始容量(16)” 和 装填因子(.75)
+ HashMap的bucket数组大小为2的幂，其初始化的大小为最接近(大于)的数。
+ 当HashMap中存有的个数大于容量x装填因子后扩容
...
+ 哈希值的使用不同:
  + Hashtable(prime): hash = key.hashCode(); index = (hash & 0x7fffffff) % tab.length;
  + hashmap(power of 2): int h;hash = (key==null)?0:(h=hashCode())^(h>>16);index=(tab.length-1)&hash;
  + 在hashmap中为了防止除了高位都相同的hash码碰撞，进行了高位和低位间的异或操作
...
+ 使用InheritableThreadLocal可以实现多个线程访问ThreadLocal的值(在子线程创建过程中会复制父中的数据)
*** 原子类
*** Lock 类
+ lock 更加灵活
+ 可提供多种形式的锁方案：
...
*** JDBC
**** 什么是JDBC
+ Java数据库连接(Java DataBase Connection)可以用来操作关系型数据库
+ JDBC接口及相关类在java.sql包和javax.sql包里
+ 可以用来连接数据库，执行SQL查询，存储过程，并处理返回的结果
+ JDBC接口让Java程序和JDBC驱动实现了松耦合，使得切换不同的数据库更加简单
**** JDBC是如何实现Java程序和JDBC驱动的松耦合的
+ JDBC API使用Java的反射机制来实现Java程序和JDBC驱动的松耦合
+ 使用Class.forName()动态加载驱动类，并注册到DriverManager类中
+ 接着使用DriverManager.getConnection方法来调用所注册的驱动方法
**** 步骤
 + 加载驱动
 + 创建连接
 + 创建语句
...
**** Resultset
+ Resultset对象维护了一个游标，指向当前的数据行
+ 调用next方法会下移一行，如果没有更多数据则返回false
+ 如果要设置可回滚或可更新的Resultset，需要在创建语句时传入相应的参数
+ 使用getInt,getString等参数来获取相应的列数据
+ 当执行一条查询语句时，查询到的数据是在数据库的缓存中维护的，Resultset引用的就是数据库缓存中的结果
+ setFetchSize参数设置的就是数据库驱动每次取的数据
+ setMaxRows用来限制返回的数据集的行数
**** 事务处理
+ setAutoCommit(false)
+ commit()
+ rollback()
...
+ Clob:大字符对象
**** JDBC的DataSource
+ DataSource即数据源，其功能比DriverManager更强大，除了可以用来创建数据库连接外，还能：
...
**** DBCP
+ Apache的DBCP用来解耦获取连接的代码和驱动特定的DataSource
+ 它提供的DataSource实现了应用程序和不同JDBC驱动间的抽象层
**** RowSet
+ RowSet用于存储查询的数据结果，其继承自ResultSet
+ RowSet对象默认是可滚动，可更新的
+ 提供了Java Bean的功能，可以通过setter和getter来设置和获取属性，其使用了事件驱动模型，可以给注册的组件发送通知
+ 它是离线的，更轻量级，更易于在网络间传输
*** Java查询语句
+ Statement：用于通用查询
...
+ 节省创建连接，释放连接资源
**** 自定义连接池
+ 实现javax.sql.DataSource接口(两个重载的getConnection方法)
+ 用户用完Connection后，不能调用Connection.close方法，而要使用连接池提供的关闭方法
+ 也可以使用动态代理来增强Connection.close分法
...
** Servlet
*** Servlet和CGI
+ Servlet存在于服务器进程中，通过多线程的方式运行其service()方法，一个实例可服务于多个请求
+ Servlet是在容器中运行的，其将用户的请求传递给Servlet程序，并将Servlet的响应回传给用户
+ Servlet实例不会销毁
+ CGI对每个请求都产生新的进程，服务完以后就销毁
+ Fast CGI解决了CGI效率上的问题
*** Servlet接口
+ void init(ServletConfig config) throws ServletException web容器运行其Servlet并对其初始化
+ void service(ServletRequest req, ServletResponse resp) throws ServletException, java.io.IOException
+ void destroy() 服务器关闭或项目卸载时销毁
+ java.lang.String.getServletInfo()
+ ServletConfig getServletConfig() 
*** Servlet方法
+ getParameter方法可以通过参数名获得参数值
+ 如果有多个值的参数，可以通过请求对象的getParameterValues方法获得
+ 也可以通过对象的getParameterMap来获得一个参数名和参数值的映射
*** 获取用户配置的初始化参数及服务器上下文参数
+ 可以通过重写Servlet接口的init(ServletConfig)方法或通过ServletConfig对象的getInitParameter方法来获取Servlet的初始参数
+ 通过ServletConfig对象的getServletContext方法获取ServletContext对象，并通过对象的getInitParameter方法来获取服务器上下文参数
+ Servlet对象也可以在处理用户请求的方法中通过请求对象的getServletContext方法获得
*** 转发和重定向
+ forward是容器中控制器的转向，是服务器请求资源
+ redirect是服务器根据逻辑，发送一个状态码，告诉浏览器重新去请求那个地址
+ forward更加高效，并且有助于隐藏实际链接
+ redirect可以访问其他服务器上的资源
*** 过滤器
**** 作用
+ 对请求或者响应进行过滤任务
**** 用途
...
+ 转换图像格式
+ 对响应内容进行压缩以减少传输量
+ 对请求或响应进行加解密处理、触发资源访问事件、对XML的输出应用XSLT
**** 主要接口
+ Fiter
+ FilterConfig
+ FilterChain
...
*** web.xml
+ 用于配置Web应用相关信息
+ 监听器，过滤器，Servlet，相关参数，会话超时时间，安全验证方式，错误页面等
+ Servlet 3规范提供了@WebServlet,@WebListener,@WebFilter等注解
*** 异步处理
+ 解决当任务处理时间很长时，Servlet或Filter会一直占用请求处理线程，导致容器遭遇线程超出的风险
+ 异步特性可以帮助节省容器中的线程，特别适合执行时间长而且用户需要得到结果的任务
+ 如果用户不需要得到结果则直接将一个Runnable对象交给Executor即可
+ 异步线程拥有ServletRequest和ServletResponse对象的引用
+ 在Servlet线程将请求转交给一个异步线程来执行业务处理后，线程本身返回至容器，此时Servlet还未生成响应数据，异步线程处理完业务后，可以直接生成响应数据或将请求转发给其他Servlet
+ Servlet 3还为异步处理提供了一个监听器，使用AsyncListener接口表示
*** 表单数据的提交
+ HTML中<form>元素的method属性指定，可以是get或post
+ doGet方法时，会在url后边显示提交的内容，不安全，而且最多只能提交1024字节
+ 可以在doGet中调用doPost方法来共用代码
** JSP
*** 内置对象
...
+ 其将JSP页面转换成一个JSP页面的实现类，这是一个实现了JspPage接口的Java类，该接口是Servlet的子接口
+ 转换成功后，容器会编译Servlet类，之后容器加载和实例化Java字节码，并执行生命周期操作
+ 对同一个JSP页面的后续请求，容器会查看这个JSP页面是否被修改过，如果改过则重新来一遍
*** JSP和Servlet
+ Servlet是一个特殊的Java程序，它运行于服务器的JVM中，能够依靠服务器的支持向浏览器提供显示内容
+ JSP本质是一个用来简化页面内容生成的类似于Servlet的Java程序。
+ Servlet的应用逻辑是在Java文件中，并且完全从表示层中的HTML分离开。
+ JSP则是Java和HTML可以组合成一个扩展名为.jsp的文件名
+ JSP更侧重于视图，而Servlet则充当控制器
*** JSP中的四种作用域
+ page:代表一个页面相关的对象和属性
+ request:代表与Web客户机发出的一个请求相关的对象和属性，一个请求可跨越多个页面，涉及多个Web组件
+ session:代表与某个用户与服务器建立的一次会话相关的对象和属性，与某个用户相关的数据应放在用户自己的session中
+ application:代表与整个Web应用程序相关的对象和属性
*** 如何实现JSP或Servlet如何实现单线程
+ JSP: <%@page isThreadSafe="false"%>
+ Servlet: 自定义的Servlet实现SingleThreadModel标识接口
+ 设置成单线程后，会导致严重的性能问题
*** Model1和Model2
+ Model1是以页面为中心的Java Web开发，使用JSP+JavaBean技术将页面显示逻辑和业务逻辑分开，JSP实现页面显示，JavaBean对象用来保存数据和实现业务逻辑
+ Model2是基于MVC架构模式的开发模型，实现了模型和视图的彻底分离，Servlet，JavaBean，JSP
** 网络应用模式
+ B/S:Browser,Server;可以视为特殊的C/S，却避免了安装、配置、升级客户端带来的各种开销
+ C/S:Client;
+ P2P:成千上万彼此连接的计算机处于对等的地位，每台计算机既能充当网络服务的请求者也能对其他计算机的请求作出响应
** 实现会话跟踪的技术
+ URL重写：在URL中添加用户会话信息作为请求的参数，或者将唯一的会话ID添加到URL结尾以标识一个会话
+ 设置表单隐藏域
+ 上述两种方法很难处理跨越多个页面的信息传递
+ cookie:
  + 基于窗口的，浏览器关闭后就没了
  + 将信息存在临时文件中，并设置存在时间
  + 在cookie中不能存放敏感信息，cookie中存储的数据量有限，不能将过多的内容存储cookie中
+ HttpSession：当一个对象第一次访问某个网站时会自动创建HttpSession，并存放在服务器的内存中，每个用户访问自己的HttpSession，存放的对象最好实现Seraiblizable接口以便序列化到文件
+ HTML5中允许Web Storage技术通过JS来保存数据，比如通过localStorage和sessionStorage保存用户会话
** Web Service
+ 从表面上看，Web Service是一个应用程序，它向外界暴露出一个能够通过Web进行调用的API
+ 它基于HTTP协议传输数据
+ 通过编程方法透明地调用这个应用，运行在不同机器上的不同应用无需附近的硬件或软件
+ 是SOA(Service-Oriented Architecture-将应用程序的不同功能单元通过中立的契约联系起来)的一种较好的解决方案，更多的是一种标准，而不是具体的技术
*** 相关概念
+ WSDL(Web Service Description Language)：Web服务描述语言，是一个基于XML的关于如何与Web服务通信和使用的服务描述
+ UDDI(Universal Description, Discovery and Integration)：统一描述，发现和集成，是一个基于XML的跨平台的描述规范，是访问各种WSDL的门面
*** 主流web服务实现方法
+ SOAP(Simple Object Access Protocol)：简单对象访问协议，是Web Service中交换数据的一种协议规范，成熟度高，安全性好
+ REST(Representation State Transfer)：简单易用，效率高
*** Java中相关Web Service规范
+ JAXM(JSR 67)：定义了发送和接受消息所需的API，相当于Web Service的服务端
+ JAX-RS(JSR 311 & JSR 339 & JSR 370)：是Java针对REST风格的一套Web Service规范。REST完全依赖HTTP协议完成Web Service，而不像SOAP那样之利用了HTTP的传输特性，定位和操作都由SOAP协议完成(SOAP因为太笨重而渐渐淘汰)
** 持久层
*** ORM
+ 对象关系映射(Object-Relational Mapping)是一种为了解决程序的面向对象模型与数据库关系模型不匹配的技术
+ 通过使用描述对象和数据库直接映射的元数据(注解或XML)来将程序中的对象自动持久化到关系型数据库或将关系型数据库中的行转换为Java对象
*** 持久层
+ 将内存中的数据保存到关系型数据库，文件系统，消息队列等提供持久化支持的设备中
**** 目标
+ 数据存储逻辑的分离，提供抽象化的数据访问接口
+ 数据访问底层实现的分离，可在不修改代码的情况下切换底层实现
...
+ Spring Data
*** Hibernate
**** SessionFactory和Session是否线程安全
+ SessionFactory对应Hibernate数据存储的概念，是线程安全的，一般只在启动的时候构建
+ 对于应用程序，最好将SessionFactory通过单例模式封装以便访问
+ Session是一个轻量级非线程安全的对象，表示与数据库交互的一个工作单元
+ Session是持久层服务对外提供的主要接口，会延迟获取数据库连接
+ 为避免创建太多的session，可以使用ThreadLocal将session和当前线程绑定
**** Session的load和get方法
+ 如果没有找到符合条件的记录，get方法返回null,load方法抛出异常
+ get方法直接返回实体类对象，load方法返回实体类对象的代理
**** Session中的方法
+ Hibernate对象有三种状态：瞬时态(transient)、持久态(persistent)和游离态(detached)
+ 瞬时态的实例可以调用save,persist或saveOrUpdate方法变成持久态
+ 游离态的实例可以通过update,saveOrUpdate,lock或replicate变成持久态
+ merge方法可以完成save和update方法的功能，它的意图是将新的状态合并到已有的持久化对象上或创建新的持久化对象
+ update是把已经更改过的托管对象变成持久状态，lock是把一个没有更改过的托管对象变成持久状态
+ persist把一个瞬时态的实例持久化，但是不保证标识符立即填入持久化实例，可能推迟到flush的时间
+ persist保证在事务外部被调用时，不触发INSERT语句，而save方法不保证
+ save方法要返回标识符，所以会立即执行INSERT，无论是在事务内部还是外部
**** Session加载实体对象的过程
+ Session在调用数据库查询功能之前，首先在一级缓存中通过实体类型和主键查找
+ 如果未命中，接着在NonExists(黑名单)记录中查找，如果有则返回null
+ 接下来查找二级缓存
+ 如果均未命中，则发出SQL语句，若无记录则添加到Session的NonExists中
+ 根据映射配置得到ResultSet，并创建实体对象
+ 将对象添加到Session(一级缓存)中管理
+ 如果有拦截器，则执行其onLoad方法(拦截初始化的方法)
+ 如果开启了二级缓存，则添加
+ 返回对象
**** Query接口中的list方法和iterate方法
+ list方法无法利用一级和二级缓存(对缓存只写不读),只能在开启查询缓存的前提下使用查询缓存
+ iterate方法可以充分利用缓存，特别适合只读或读取频繁的场景
+ list方法不会引起N+1差异问题而iterate会(从含有n个对象的表中读取包含的对象，需要先找出所以对象1+依次查找关联对象n次select，当不需要关联数据时依旧会读取)
**** 分页查询
开发人员只需要提供HQL语句（Session的createQuery方法)或查询条件(Session的createCriteria方法)，
设置查询其实行数(Query或Criteria接口的setFirstResult方法)和最大查询行数(Query或Criteria接口的setMaxResult方法)，
并调用Query或Criteria接口的list方法，Hibernate会自动生成分页查询的SQL语句。
**** 乐观锁和悲观锁的机制
+ Hibernate中通过Session的get和load方法从数据库加载对象时可以通过参数指定使用悲观锁(query.setLockMode("user",LockMode,UPGRADE);)
+ 而乐观锁在通过给实体类加整型的版本字段再通过XML或@Version注解进行配置
**** Hibernate的检索策略
***** 类级别检索策略
+ 立即检索(默认)
+ 延迟检索：通过lazy属性来确定，仅适用load检索
***** 关联级别检索
一对多和多对多
+ 立即检索(一对多的默认检索策略,N+1问题)：此时lazy=false,out-join=false，如果关联集合是无用的，则不要用这种方式
+ 延迟检索：此时lazy=true,out-join=false
+ 迫切左外连接检索(利用了sql的外连接)：lazy=false,out-join=true，只适用于依靠id检索(load,get)，而不适合query的集合检索(采取立即检索策略)

多对一和一对一关联
<many-to-one>中的outer-join属性
+ auto:默认，lazy=true则延迟检索，否则迫切左外连接检索
+ true:均为迫切左外连接检索
+ false:lazy=true则延迟检索，否则立即检索

<one-to-one>中的constrained属性
+ 如果采用懒加载机制，必须把constrained属性设为true
**** 延迟加载
+ 延迟加载就是并不是在读取的时候而是等使用的时候才加载数据
+ Hibernate使用虚拟代理机制实现延迟加载
+ 使用Session的load加载数据或者一对多情况下的延迟加载都是使用的虚拟代理，返回的是实体对象的代理

延迟加载和session的关闭之间存在矛盾
+ 关闭延迟加载，但会增加查询开销
+ 在session关闭前获取查询对象，Hibernate.isInitialized和Hibernate.initialize方法
+ 使用拦截器或过滤器延长Session的生命周期直到视图获得数据(HibernateUtil.getCurrentSession, closeCurrentSession)
+ Spring 的 OpenSessionInViewFilter 过滤器，为每个请求自动绑定一个Hibernate Session
**** 实体对象的三种状态及其转换
+ 瞬时态：当new一个实体对象后，这个对象处于瞬时态，即这个对象只是保存临时数据的内存区域，如果没有变量引用则会被垃圾回收，通过Session的一些方法可以和数据库关联，从而转换为持久对象。
+ 持久态：持久态对象的实例在数据库中有对应的记录，并拥有一个持久化标识(ID)，当数据库中对应元素删除后，该对象将变成删除态(旧版的中为瞬时态)
+ 游离态：当Session进行了close,clear,evict或flush后，实体对象由持久态变成游离态
**** 继承映射
+ 单表继承：每课类继承树使用一个表(只有一个表)
+ 类表继承：每个子类生成一个表
+ 具体类继承：每个类生成一个表
**** 常见优化策略
+ 制定合理的缓存策略(二级缓存，查询缓存)
...
+ 设定合理的批处理参数
+ 如果可以，选用UUID作为主键生成器
+ 如果可以，使用基于版本号的乐观锁代替悲观锁
+ 在开发过程中，开启hibernate.show_sql查看生成的SQL
+ 考虑数据库本身的优化
**** 缓存机制
+ 一级缓存：默认总是有效的，当应用保存或修改持久化实体时，Session并不会立即把这种改变提交到数据库，而是缓存在Session中，除非调用了Session的flush或close方法
+ 二级缓存：SessionFactory级别的二级缓存是全局的，所有Session可以共享这个二级缓存，SessionFactory缓存访问过的该实体类的每个对象，除非缓存的数据超过了指定的缓存空间，不过默认是关闭的
+ 一级缓存和二级缓存都是对整个实体进行缓存，不会缓存普通对象，如果希望对普通属性进行缓存，可以使用查询缓存
+ 查询缓存是对HQL或SQL语句以及它们的查询结果作为键值对进行缓存，默认是关闭的
**** Criteria和DetachedCriteria
+ Criteria是一个直观的，可扩展的条件查询API
+ Criteria是由Session的createCritia方法创建的，离开了创建它的Session就无法使用
+ DetachedCriteria不需要Session就能创建，在需要查询时再和具体的Session绑定
**** @OneToMany注解的mappedBy属性
+ 通常情况下，一对多关联映射都由多的一方来维护
+ mappedBy标签定义在被拥有方，指向拥有方属性(自动维护方)
+ mappedBy跟joinColumn/joinTable总是处于互斥的一方
+ 如果不加mappedBy则会被认为是两个独立的关系
**** 多对多关联
+ 中间表
+ 两个多对一关联
** Spring
*** IOC DI
+ 控制反转：将对组件对象控制权，从程序代码本身转移到外部容器上
+ 依赖注入：查找资源的逻辑从应用组件的代码中抽取出来，交给容器来完成(setter方法，构造器，接口注入，Spring支持前两种)
*** Bean的作用域
+ singleton (一个容器中只能存在一个实例)
+ 在Spring中，可以对DAO采用单例模式，因为对其使用了ThreadLocal进行了特殊处理
+ prototype(每次从容器中调用Bean，都会返回一个新的实例)
+ request(每次HTTP请求都会创建一个新的Bean)
+ session
+ globalSession(同一个全局Session共享一个Bean，和Portlet应用相关)
*** Bean的生命周期
...
+ 如果Bean实现了InitializingBean接口，则调用afterPropertySet方法
+ 当销毁Bean实例时，如果Bean实现了DisposableBean接口，则调用destroy方法
*** Bean何时被创建
+ 工厂类设置为ApplicationContext的非lazy-init的单例bean在应用上下文环境被创建时被构造
+ 其余均在第一次被请求时被构造
*** AOP
+ 面向切面编程(Aspect-Oriented Programming)，用来关注分散在对象，类或方法中的横切关注点
+ 通常，事务，日志，安全等关注都是应用中的横切关注功能
**** 概念
+ 连接点(Joinpoint)：程序执行的某个特定位置(某个方法调用前后，异常抛出时)
+ 切点(Pointcut)：如果连接点相当于数据中的记录，那么切点相当于查询条件，一个切点可以匹配多个连接点
+ 增强(Advice)：织入到目标类连接点的一段程序代码(接口:BeforeAdvice,AfterReturningAdvice,ThrowsAdvice)
+ 切面(Aspect)：切面是由切点和增强(引入)组成的，它包括了对横切关注功能的定义，也包括了对连接点的定义
+ 引入(Introduction)：引介是一种特殊的增强，它为类添加一些属性和方法(动态添加接口的功能)
...
+ byType
+ constructor
+ autodetect：默认构造器，byType

限制
+ 如果使用了构造器注入或者setter注入，将覆盖自动装配的依赖关系
+ 基本数据类型，字符串字面量，类字面量无法使用自动装配
+ 自动装配不够精确，优先考虑使用显式的装配来进行更精确的依赖注入
*** 配置Bean
+ 注解：@Component,@Controller,@Service,@Repository，这几个并没有本质区别
+ 方式：XML,注解，Java程序
*** 框架优点
+ 非侵入式：支持基于POJO的编程模式，不强制性的要求实现Spring框架的接口或继承Spring框架中的类
+ IoC容器：IoC容器帮助应用程序管理以及对象之间的依赖关系
+ AOP：将所有横切关注功能封装到切面中，通过配置的方式将横切关注功能动态添加到目标代码
+ MVC：Spring的MVC框架为Web表示层提供了更好的解决方案
+ 事务管理：Spring提供多种持久层技术，并且为其提供了声明式的事务管理，在不需要任何一行代码的情况下能够完成事务管理
+ 其他：Spring为Java企业级开发提供了一站式选择
*** 事务管理类型
+ Spring支持编程式和声明式事务管理
+ 编程式灵活性高，声明式更轻量
+ 事务分为全局事务和局部事务
+ 全局事务由应用服务器管理，需要底层服务器JTA支持
+ 局部事务和底层采用的持久化方案有关，例如JDBC需要Connection来操作事务，而Hibernate需要Session
*** Spring MVC的工作原理
+ 客户端的所有请求都交给前端控制器DispatcherServlet来处理
+ DispatcherServlet收到请求后，将根据请求的信息(包括URL,HTTP协议方法，请求头，请求参数，Cookie等)以及HandlerMapping的配置找到处理该请求的Handler
+ 此时Spring会通过HandlerAdapter对该处理器进行封装
+ HandlerAdapter是一个适配器，它用统一的接口对各种Handler中的方法进行调用
+ Handler完成用户请求的处理后，会返回一个ModelAndView对象给DispatcherServlet
+ ModelAndView的视图是逻辑视图，之后DispatcherServlet借助ViewResolver将其解析成真实视图对象
+ 当得到真正的视图对象之后，DispatcherSevlet会利用视图对象对模型数据进行渲染
+ 客户端得到响应
** 性能优化技术
*** 架构优化
+ 分层
+ 分割：按功能和服务分成模块
+ 分布式：静态资源，数据，分布式计算框架
+ 集群：有更多的服务器提供相同的服务，更好的支持并发
+ 缓存：CDN,反向代理，热点数据；分布式缓存
+ 异步：生产者消费者；提高可用性，响应速度，削峰
+ 冗余：提高可用性
*** 前端优化
+ 浏览器访问优化：减少HTTP请求，使用浏览器缓存
+ CDN加速
+ 反向代理：缓存，负载均衡
*** 应用服务器优化
+ 分布式缓存：哈希表，二八定律
...
+ 代码优化
  + 多线程
  + 非阻塞I/O：使用单线程和非阻塞I/O是目前公认比多线程更能充分发挥服务器性能的应用模式
  + 资源复用：单例，对象池
* Python
** Python 和多线程
...
*** 常见攻击方式
**** XSS攻击
+ 跨站脚步攻击(Cross Site Script)是向网页中注入恶意脚本在用户浏览网页时在用户浏览器中执行攻击的方式
+ 防范措施主要有两个：消毒(对危险字符进行转义)和HttpOnly(防范XSS攻击者窃取Cookie数据)
**** SQL攻击
+ 最主要的注入式攻击方式
**** CSRF攻击
+ 跨站请求伪造(Cross Site Request Forgery)是攻击者通过跨站请求，以合法身份进行非法操作
+ 利用浏览器的Cookie或服务器的Session来盗取用户身份
+ 防范措施有：令牌，验证码，请求头中的Referer
* 数据库
** 数据库范式
*** 范式
